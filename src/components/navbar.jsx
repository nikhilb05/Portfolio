...
const Navbar = () => {
    ...
    return (
        <div className='flex justify-between items-center w-full h-20 fixed px-4 text-white bg-black'>
            <div>
                <h1 className='text-5xl font-signature ml-2'>Nikhil</h1>
            </div>
            ...
            <div onClick={() => setHam(!ham)} className='cursor-pointer pr-4 z-10 md:hidden'>
            ...
            {ham && (
                ...
                <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen text-gray-500 bg-gradient-to-b from-black to-gray-800'>
                    ...
                    {links.map(({ id, link }) => (
                        <li key={id} className='px-4 cursor-pointer capitalize py-6 text-4xl hover:scale-125 duration-200'>
                            <Link onClick={() => setHam(!ham)} to={link} smooth duration={500}>
                            {link}
                            </Link>
                        </li>
                    ))}
